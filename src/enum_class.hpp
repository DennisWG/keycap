...
template <typename T>
constexpr typename std::enable_if<std::is_enum<T>::value && enable_enum_class_bitmask<T>::value, bitmask<T>>::type
 ...
 */
template <typename T>
constexpr typename std::enable_if<std::is_enum<T>::value && enable_enum_class_bitmask<T>::value, enumerator<T>>::type
...
operator&(const enumerator<T>& lhs, const enumerator<T>& rhs)
{
    using underlying_type = typename std::underlying_type<T>::type;
    return enumerator<T>{
        static_cast<T>(static_cast<underlying_type>(lhs.value) & static_cast<underlying_type>(rhs.value))};
}
 ...
 */
template <typename T>
constexpr typename std::enable_if<std::is_enum<T>::value && enable_enum_class_bitmask<T>::value, bool>::type
operator==(const T& lhs, const T& rhs)
{
    return lhs == rhs;
}
